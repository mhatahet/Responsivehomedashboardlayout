...
const data = [
  { activity: 'Calls', count: 342 },
  { activity: 'Meetings', count: 189 },
  { activity: 'Demos', count: 156 },
  { activity: 'Proposals', count: 98 },
  { activity: 'Follow-ups', count: 267 },
];
...
export function ActivitiesChart({ dateRange, customStartDate, customEndDate }: ActivitiesChartProps) {
  ...
  const handleExportCSV = () => {
    const csvContent = 'Activity,Count\n' + 
      data.map(row => `${row.activity},${row.count}`).join('\n');
    const blob = new Blob([csvContent], { type: 'text/csv' });
    const url = window.URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = 'top-activities.csv';
    ...
    <div className="bg-white rounded-xl p-6 border border-neutral-200 relative" ref={chartRef}>
      <div className="mb-6">
        <div className="flex items-start justify-between mb-1">
          <h3 className="text-neutral-900">Top Activities</h3>
        ...
        <div className="flex items-center justify-between">
          <p className="text-neutral-500">Activity breakdown</p>
